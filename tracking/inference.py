    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for p in self.particles:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                if emissionModel[trueDistance] > 0:
                    newBelief[p] += emissionModel[trueDistance]
            if newBelief.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                self.particles = []
                newBelief.normalize()
                self.particles = util.nSample(newBelief.values(),newBelief.keys(),self.numParticles)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for num in range(self.numParticles):
            self.particles.append(permutations[num % len(permutations)])
    ...
    def observeState(self, gameState):
        ...
        for ghostIndex in range(self.numGhosts):
            if noisyDistances[ghostIndex] == None:
                for index in range(len(self.particles)):
                    ...
                    self.particles[index] = self.getParticleWithGhostInJail(p, ghostIndex)

        if self.numGhosts == sum([d == None for d in noisyDistances]):
            return
        ...
        for particle in self.particles:
            ...
            for ghostIndex in range(self.numGhosts):
                if noisyDistances[ghostIndex] != None:
                    ghost = particle[ghostIndex]
                    dist = util.manhattanDistance(ghost, pacmanPosition)
                    n = n * emissionModels[ghostIndex][dist]
            particleDist[particle] += n
        ...
        for _ in range(self.numParticles):
            sample = util.sample(particleDist)
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for ghostIndex in range(self.numGhosts):
                ghostPosDistr = getPositionDistributionForGhost(setGhostPositions(gameState, oldParticle), ghostIndex, self.ghostAgents[ghostIndex])
                newParticle[ghostIndex] = util.sample(ghostPosDistr)
            "*** END YOUR CODE HERE ***"
