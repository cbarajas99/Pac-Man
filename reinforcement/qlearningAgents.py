...
class QLearningAgent(ReinforcementAgent):
    ...
    def getAction(self, state):
        ...
        "*** YOUR CODE HERE ***"
        if len(legalActions) != 0:
            if util.flipCoin(self.epsilon) == True: #True : epsilon; False: 1 - epsilon
    ...
    def update(self, state, action, nextState, reward):
        ...
        bellmanError = (reward + self.discount * self.QValue[(nextState, nextAction)]) - self.QValue[(state, action)]
        self.QValue[(state, action)] = self.QValue[(state, action)] + self.alpha * (bellmanError)
